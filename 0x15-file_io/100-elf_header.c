 ...
 * @e_ident: pointer to an array with ELF magic nos
 ...
 * @e_ident: pointer  to array with the ELF magic nos
...
/**
 * print_abi -> printss the ABI version of an ELF header
 ...
 * Description: if the cant be closed exit code 98
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
